...
import { getRequestByDriver, getRequestByDriverAll } from '@/lib/appwrite'
...
const Ride = () => {
    ...
    useEffect(() => {
        const fetchnear = async ()=>{
            try {
                const results = await getRequestByDriverAll(userData?.$id!)
    ...
    const getStatusIcon = (status: string): { name: keyof typeof Ionicons.glyphMap, color: string } => {
      ...
      return (
        <View style={[styles.container, isRTL && styles.containerRTL]}>
          <Text style={{textAlign : 'center' ,  fontSize : 25 , margin : 20 }} className='font-Poppins-bold'>{t('deliveryHistory')}</Text>
